...
public class HeroController : MonoBehaviour {
	...
	void ClickToMove()
	{
		...
		if(Input.GetMouseButton(0) && target!=null && dontClick == false)
		{
			//if disable auto attack it can attack 1 time
			if(!autoAttack)
			{
	...
	public void GetDamage(float targetAttack,float targetHit,float flinchRate,
        GameObject atkEffect,AudioClip atksfx)
    {
		//Calculate Hit
	...
	public void GetCastID(int caseID){
		...
		if(ctrlAnimState != ControlAnimationState.Cast 
            && ctrlAnimState != ControlAnimationState.ActiveSkill 
            && ctrlAnimState != ControlAnimationState.Death && 
			ctrlAnimState != ControlAnimationState.Attack)
		{
			castid = caseID;
